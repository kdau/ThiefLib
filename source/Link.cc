...
bool
Link::subscribe (Flavor flavor, const Object& source, const Object& _host)
{
	IRelation* relation = SInterface<ILinkManager> (LG)->GetRelation
		(flavor.number);
	if (!relation) return false;
	Object host = (_host == Object::SELF) ? source : _host;
	return SService<IDarkHookScriptService> (LG)->InstallRelHook
		(host.number, kDHNotifyDefault, relation->Describe ()->szName,
			source.number);
}

bool
Link::unsubscribe (Flavor flavor, const Object& source, const Object& _host)
{
	IRelation* relation = SInterface<ILinkManager> (LG)->GetRelation
		(flavor.number);
	if (!relation) return false;
